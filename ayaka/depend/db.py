...
'''
    __table_name__ = ""
    __primary_key__ = sql.PrimaryKey
    __json_key__ = sql.JsonKey
    ...
    @classmethod
    def drop_table(cls):
        sql.drop_table(cls.__table_name__)

    @classmethod
    def create_table(cls):
        '''[已过时的API] 根据数据类型自动创建表'''
        sql.create_table(cls.__table_name__, cls)

    @classmethod
    def replace(cls, data: Self):
        sql.insert_or_replace(cls.__table_name__, data, "replace")

    @classmethod
    def replace_many(cls, datas: List[Self]):
        sql.insert_or_replace_many(cls.__table_name__, datas, "replace")

    @classmethod
    def insert(cls, data: Self):
        sql.insert_or_replace(cls.__table_name__, data, "insert")

    @classmethod
    def insert_many(cls, datas: List[Self]):
        sql.insert_or_replace_many(cls.__table_name__, datas, "insert")
    ...
    def select_many(cls, **params) -> List[Self]:
        ...
        return sql.select_many(cls.__table_name__, cls, extra)
